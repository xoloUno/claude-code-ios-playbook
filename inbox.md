...
### 2026-05-03 — Playbook (deferred shotsmith CLI work)

**Category:** suggestion
**Context:** While adopting the `manual-captures/` two-input-tree pattern
into the playbook (rule + Fastfile compose lane + bootstrap-emitted
`fastlane/manual-captures/README.md`), two shotsmith CLI changes were
considered and deferred:

1. **`shotsmith mirror` subcommand** to replace the manual `cp -r es-ES
   es-MX` workflow for identical-content locale pairs (es-ES↔es-MX,
   fr-FR↔fr-CA, pt-PT↔pt-BR). Saves ~3 min per release per pair. Scope:
   `shotsmith/mirror.py` + `.mirrored-from` sentinel + `verify` warning
   on source-newer-than-mirror + tests. Roughly 150 LOC + tests, version
   bump to 0.2.0.

2. **`manual_inputs` config block in `shotsmith/config.json`** to move the
   `manual-captures/<locale>/` → `raw/` staging step from the Fastfile
   lane into shotsmith itself. Today the staging is ~10 lines of Ruby in
   `:compose_screenshots`. Inlining into shotsmith means each project's
   Fastfile shrinks and the contract for "tracked manual inputs" lives
   in one place.

**Lesson:** Both are real wins but not load-bearing. Manual `cp -r` and
the small Ruby staging block work fine for now; both deferrals avoid
introducing drift risk (mirroring) or over-coupling shotsmith to one
project layout (manual_inputs).

**Suggested action:** Revisit when:
- The set of mirrored locale pairs grows past ~3, OR a project hits a
  `cp -r` mistake (mirror got stale because source was recaptured but
  mirror wasn't), OR recapture cadence increases enough that 3 min /
  release adds up.
- A second project adopts the two-input-tree pattern and the Fastfile
  staging block starts diverging across projects (signal that it should
  move into shotsmith).

Both are scoped for a focused shotsmith CLI session, not a playbook
session — they're shotsmith package changes, not playbook docs/rules.
